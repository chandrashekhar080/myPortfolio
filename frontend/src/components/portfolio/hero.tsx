...
const DEFAULT_STACK = ["React.js", "Next.js", "Node.js", "Express.js", "MongoDB", "Tailwind CSS"];

export function Hero() {
  const content = useContent();
  const { profile, typedRoles, projects, skillGroups } = content;
  const typed = useTypedText(typedRoles);
  const reduce = useReducedMotion();

  // The marquee and the chip grid show the most-used technologies across the
  // published projects, so adding a project keeps the hero in sync.
  const stack = useMemo(() => {
    const counts = new Map<string, number>();
    for (const project of projects) {
      for (const tech of project.tech) counts.set(tech, (counts.get(tech) ?? 0) + 1);
    }
    const ranked = [...counts.entries()].sort((a, b) => b[1] - a[1]).map(([tech]) => tech);
    return ranked.length >= 6 ? ranked.slice(0, 6) : DEFAULT_STACK;
  }, [projects]);

  // "Ganesh Nagar, Indore, Madhya Pradesh, India" → "Indore, India" for the stat tile.
  const locationParts = profile.location.split(",").map((part) => part.trim()).filter(Boolean);
  const shortLocation =
    locationParts.length > 2
      ? [locationParts.at(-3), locationParts.at(-1)].filter(Boolean).join(", ")
      : profile.location;

  const focusAreas = skillGroups.slice(0, 3).map((group) => group.label);
  const snippet = [
    `const developer = {`,
    `  role: ${JSON.stringify(profile.title)},`,
    ...(profile.company ? [`  company: ${JSON.stringify(profile.company)},`] : []),
    `  stack: ${JSON.stringify(stack.slice(0, 4))},`,
    ...(focusAreas.length ? [`  focus: ${JSON.stringify(focusAreas)},`] : []),
      ...
      <div className="section-shell relative grid items-center gap-12 pt-4 lg:grid-cols-[1.15fr_0.85fr]">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <ActionButton onClick={() => downloadResume(content)}>
          ...
          <dl className="mt-10 grid max-w-lg grid-cols-2 gap-x-6 gap-y-5 sm:grid-cols-3">
            ...
            <div>
              ...
              <dd className="mt-1 font-display text-xl font-bold">{projects.length}+</dd>
            </div>
          ...
          <div className="glass rounded-3xl p-5 sm:p-6">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-2 truncate font-mono text-[0.65rem] text-muted-foreground">
                {(profile.name.split(" ")[0] || "Developer")}.jsx
              </span>
            </div>
