...
import { API_URL } from "@/lib/site-content";
...
export function Contact() {
  ...
  /**
   * Posts to the backend, which stores the message for the admin inbox. With no
   * API configured the form falls back to opening the visitor's mail client, so
  ...
  async function submit(form: HTMLFormElement) {
    ...
    if (!API_URL) {
      ...
      window.location.href = `mailto:${profile.email}?subject=${encodeURIComponent(data.subject)}&body=${body}`;
      return;
    }
