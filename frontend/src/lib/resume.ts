...
function resumeFileName(profile: SiteContent["profile"], extension: string) {
...
function profileLinks(profile: SiteContent["profile"]) {
  return [profile.linkedin, profile.github].filter(Boolean) as string[];
}

function buildResumeHtml(content: SiteContent) {
  const { profile, experience, education, certifications, projects, skillGroups } = content;
  const summary = content.resumeSummary;

  const contact = [profile.location, profile.email, `${profile.phone}, ${profile.phoneAlt}`]
    .map(escapeHtml)
    .join(" · ");
  const links = profileLinks(profile).map(escapeHtml).join(" · ");
...
function downloadResumeText(content: SiteContent) {
  const { profile, experience, education, certifications, projects, skillGroups } = content;
  const summary = content.resumeSummary;
  ...
  a.href = url;
  a.download = resumeFileName(profile, "txt");
  a.click();
 ...
 */
export function downloadResume(content: SiteContent) {
  ...
  if (!win) {
    downloadResumeText(content);
    return;
  }

  win.document.write(buildResumeHtml(content));
  win.document.close();
